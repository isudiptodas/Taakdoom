                    ...
                    <div className={`w-full hidden px-4 xl:px-2 xl:flex justify-center items-center`}>
                        {navLinks.map((item, index) => {
                            return <div key={index} className={`w-auto flex flex-col justify-center items-center gap-2`}>
                                <Link href={item.url} onClick={() => setCurrentSection(item.url.split('#')[1])} className={`w-auto ${currentSection === item.url.split('#')[1] ? "font-bold" : "font-light"} text-white px-5`}>{item.name}</Link>
